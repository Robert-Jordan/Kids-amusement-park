...
import { CategoryState, Service } from './types';
...
const categoriesReducer: Reducer<CategoryState> = (state: CategoryState = initialState, action) => {
    switch (action.type) {
        case actions.SORT_BY_PRICE:
            ...
            let sortedPriceArr = action.payload.direction === "asc" ?
                sortByPriceAsc(services) :
                sortByPriceDesc(services);
            return {
                ...state,
...
const filterByCity = (list: Service[], filter: string) => {
    let filteredList = new Array<Service>();
...
const filterByCategory = (list: Service[], filter: string) => {
    let filteredList = new Array<Service>();
    list.map((categories) => {
...
const sortByPriceAsc = (arr: Service[]) => {
    return arr.sort(function (a, b) {
        if (a.price.minimalPrice > b.price.minimalPrice) return 1;

        if (b.price.minimalPrice > a.price.minimalPrice) return -1;

        return 0;
    })
}

const sortByPriceDesc = (arr: Service[]) => {
    return arr.sort(function (a, b) {
        if (a.price.minimalPrice > b.price.minimalPrice) return -1;
