...
const CheckoutOrderComponent = () => {
    ...
    const increaseVisitorsCount = (param: string) => {
        dispatch(actions.increaseVisitorsCount({visitorType: param}));
    }
    const decreaseVisitorsCount = (param: string) => {
        dispatch(actions.decreaseVisitorsCount({visitorType: param}));
    }
                ...
                <div className="order-card">
                    <p className="order-card__city">
                        <p className="order-card__name">
                            {serviceData !== undefined && serviceData.city !== undefined && serviceData.city.name === 'Kharkiv'
                                ? 'Mall "Karavan"'
                                : 'Mall "Khreshchatyk"'}
                            <span className="order-card__date">{selectedDate !== undefined && selectedDate}</span>
                        </p>
                        {serviceData !== undefined && serviceData.city !== undefined && serviceData.city.name}
                ...
                <div className="order-room">
                    <div className="order-room__name-block">
                        <p className="order-room__name">
                            {serviceData !== undefined && serviceData.category !== undefined && serviceData.category[0].name}
                        </p>
                        <h3 className="order-room__title">{serviceData !== undefined && serviceData.title}</h3>
                    </div>
                    <div>
                        <p><span className="order-room__time">Time</span></p>
                        <h3><span className="order-room__time-value">
                            {selectedTime.startTime !== null && selectedTime.endTime !== null && (selectedTime.startTime + ' - ' +  selectedTime.endTime)}
                        </span></h3>
                    </div>
                </div>
                <div>
                        ...
                        {serviceData !== undefined && serviceData.price !== undefined &&
                            <>
                                ...
                                <div>
                                    ...
                                    <p className="room-wrapper__val 200" >
